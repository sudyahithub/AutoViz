    ...
    internal static class GeometrySignature
    {
        ...
        public static GeomFeatures Compute(BlockTableRecord btr, Transaction tr)
        {
            ...
            foreach (ObjectId id in btr)
            {
                ...
                try { boxes.Add(e.GeometricExtents); } catch { }
                ...
                else if (e is Polyline pl)
                {
                    ...
                    for (int k = 0; k < n - 1; k++)
                    {
                        Point2d p1 = pl.GetPoint2dAt(k);
                        Point2d p2 = pl.GetPoint2dAt(k + 1);
                ...
                else if (e is Arc ar)
                {
                    ...
                    double ang = mid + Math.PI * 0.5; // tangent
            ...
            int sum = 0;
            for (int i = 0; i < 8; i++) sum += gf.AngleHist8[i];
            if (sum > 0)
            {
                for (int i = 0; i < 8; i++)
                    gf.AngleHist8[i] = (int)Math.Round(1000.0 * gf.AngleHist8[i] / sum);
            }
        ...
        private static double AngleOf(Point3d a, Point3d b)
        {
            double dx = b.X - a.X;
            double dy = b.Y - a.Y;
            double phi = Math.Atan2(dy, dx);   // 0..π with folding below
        ...
        private static int EstimateComponents(List<Extents3d> boxes, double thr)
        {
            ...
            // rename params to avoid shadowing locals
            Action<int, int> unite = (u, v) =>
            {
        ...
        public static double GeomSimilarity(ref GeomFeatures A, ref GeomFeatures B)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                double xi = A.AngleHist8[i];
                double yi = B.AngleHist8[i];
                double denom = xi + yi;
                if (denom > 0) chi += ((xi - yi) * (xi - yi)) / denom;
            }
