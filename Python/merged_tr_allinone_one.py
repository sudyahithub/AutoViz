...
"""
DXF → CSV + Google Sheets (Apps Script Web App) + Local Previews (merged)
- Step 1: Process DXF(s) with zones (PLANNER) + layer metrics, write CSV.
- Step 2: Push rows to Google Sheets via your Web App (Detail + ByLayer).
- Step 3: Scan local folder for PNG/JPG/WEBP previews and upload by BOQ name
          using Web App op=previewByName.

Run:  python merged_dxf_pipeline.py
"""

from __future__ import annotations

import os, sys, argparse, csv, uuid, time, math, logging, io, base64
...
# Headless matplotlib for PNG previews (Detail only)
import matplotlib
matplotlib.use("Agg")
...
# numeric formatting
DEC_PLACES = 2  # default number of decimal places

# force category “PLANNER” for counted rows that belong to a zone
FORCE_PLANNER_CATEGORY = True

# ---- Local previews (Code 2) ----
LOCAL_PREVIEW_DIR = r"C:\Users\admin\Downloads\VIZ-AUTOCAD\EXPORTS\B1_SECOND FLOOR DONE\INSTANCES"
...
def _collect_planner_zones(msp) -> list[Zone]:
    ...
    for ins in msp.query('INSERT[layer=="PLANNER"]'):
        try:
            ...
            try:
                ...
                pass
            if not zname:
                zname = (getattr(ins, "effective_name", None) or
                         getattr(ins, "block_name", None) or
                         getattr(ins.dxf, "name", "") or "").strip()
    ...
    # B) Otherwise: closed PLANNER LWPOLYLINEs + nearest/inside text naming
    tmp: list[Zone] = []
...
def make_row(entity_type, qty_type, qty_value,
    ...
    return {
        ...
        "layer": layer_or_misc(layer),            # category (PLANNER)
        "zone": (zone or ""),                     # zone name
        "category1": category1 or "",             # original DWG layer
...
def iter_block_rows(msp, include_xrefs: bool,
    ...
    for ins in msp.query("INSERT"):
        try:
            ...
            # Skip PLANNER inserts themselves (they represent zone rectangles)
            ...
            out.append(make_row(
                ...
                zone=center_zone,
                category1=(ly or "").strip().lower(),     # original layer → category1
                remarks=remarks_txt
            ))
...
def push_rows_to_webapp(rows: list[dict], webapp_url: str, spreadsheet_id: str,
    ...
    """
    Sends rows to Sheets with tab-specific headers:
      - Detail: perimeter/area removed.
      - ByLayer: zone/category1/BOQ name/qty_value removed.
    """
    ...
    for idx, i in enumerate(range(0, total, batch_rows), start=1):
        ...
        if is_layer:
            ...
            data_rows = [[
                ...
                r.get("perimeter",""),
                r.get("area",""),
                "",  # Preview (we color the cell via bgColors)
                r.get("remarks",""),
            ] for r in chunk]
            images     = [""] * len(chunk)                      # no images for ByLayer
            ...
            data_rows = [[
                ...
                r.get("bbox_length",""),
                r.get("bbox_width",""),
                "",  # preview handled by web app with Drive images
                r.get("remarks",""),
            ] for r in chunk]
...
def process_one_dxf(dxf_path: Path, out_dir: Path | None,
    ...
    insert_rows = iter_block_rows(
        msp, include_xrefs, scale_to_m, target_units,
        preview_cache, zones
    )
    ...
    for root, _, files in os.walk(local_dir):
        for fn in files:
            ...
            try:
                ...
                logging.warning(f"Could not read {full}: {ex}")
    return items

def send_preview_batch(sess: requests.Session, items_batch):
    payload = {
        "op": "previewByName",
        "sheetId": GSHEET_ID,
        "tab": GSHEET_TAB,
        "driveFolderId": GS_DRIVE_FOLDER_ID,
        "items": items_batch,
    }
    r = sess.post(GS_WEBAPP_URL, json=payload, timeout=TIMEOUT)
    if not r.ok:
        raise RuntimeError(f"WebApp HTTP {r.status_code}: {r.text}")
    return r.json()

def upload_local_previews(local_dir: Path, batch_images: int = BATCH_IMAGES):
    if not local_dir.exists():
        logging.error("Local preview folder not found: %s", local_dir)
        return
    items = collect_items_from_local(local_dir)
    if not items:
        logging.info("No local preview images found.")
        return
    logging.info("Found %d local images. Uploading in batches of %d...", len(items), batch_images)
    sess = requests.Session()
    total_matched = total_wrote = 0
    for i in range(0, len(items), batch_images):
        batch = items[i:i+batch_images]
        try:
            res = send_preview_batch(sess, batch)
            matched = int(res.get("matched", 0))
            wrote   = int(res.get("wrote", 0))
            total_matched += matched
            total_wrote   += wrote
            logging.info("Image batch %d: matched %d, wrote %d", i//batch_images+1, matched, wrote)
        except Exception as ex:
            logging.error("Image batch %d failed: %s", i//batch_images+1, ex)
    logging.info("Local previews done. Total matched: %d, previews written: %d", total_matched, total_wrote)

# --------------------------- MAIN ---------------------------
def main():
    ap = argparse.ArgumentParser(description="DXF → CSV + Sheets + Local Previews (merged).")
    ap.add_argument("--dxf")                        # optional: file or folder
    ap.add_argument("--name")                       # optional: file stem inside DXF_FOLDER
    ap.add_argument("--decimals", type=int, default=None, help="Decimal places (default 2)")
    ap.add_argument("--out-dir")                    # optional override
    ap.add_argument("--out")                        # optional single CSV path
    ...
    # Local previews
    ap.add_argument("--previews-dir", default=LOCAL_PREVIEW_DIR)
    ap.add_argument("--batch-images", type=int, default=BATCH_IMAGES)
    ...
    # Finally: upload local previews (by BOQ name)
    upload_local_previews(Path(args.previews_dir), batch_images=args.batch_images)
