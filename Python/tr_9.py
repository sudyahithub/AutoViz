...
# - Per-layer metrics: OPEN length vs CLOSED perimeter+area
# - Detail (INSERT) previews: =IMAGE(DriveURL) written by the Web App
# - ByLayer previews: cell background filled with the layer color
...
# numeric formatting
DEC_PLACES = 2  # default number of decimal places for all numeric outputs
...
def _fmt_num(val, places: int | None = None) -> str:
    ...
    try:
        ...
        # normalize text->float (avoids 'nan', stray whitespace, etc.)
...
def iter_block_rows(msp, run_id, source_file, include_xrefs: bool,
    ...
    out = []; preview_cache = preview_cache or {}
    for ins in msp.query("INSERT"):
        try:
            name = getattr(ins, "effective_name", None) or getattr(ins, "block_name", None) or getattr(ins.dxf, "name", "")
            if not include_xrefs and ("|" in (name or "")): continue
            ...
            out.append(make_row(
                ...
                block_name=name, layer=getattr(ins.dxf, "layer", ""), handle=getattr(ins.dxf, "handle", ""),
                bbox_length=L_out, bbox_width=W_out, preview_b64=preview_cache.get(name, "")
            ))
...
def push_rows_to_webapp(rows: list[dict], webapp_url: str, spreadsheet_id: str,
    ...
    for idx, i in enumerate(range(0, total, batch_rows), start=1):
        ...
        payload = {
            ...
            # (IMG sizing via column/row sizes in Apps Script; no need to pass imageMode/W/H)
...
def sort_rows_for_category_blocks(rows: list[dict]) -> None:
    def _key(r):
        cat = _norm_cat(r.get("layer", "")); et = r.get("entity_type", "")
...
def process_one_dxf(dxf_path: Path, out_dir: Path | None,
    ...
    run_id = make_run_id(); source_file = dxf_path.name
...
def main():
    ap = argparse.ArgumentParser(description="DXF → CSV + Sheets upload (detail previews + layer color swatches).")
