...
# - Previews:
#     * Detail sheet (INSERT rows): tiny line previews from block geometry
#     * ByLayer sheet (LAYER_SUMMARY rows): solid color swatch of that layer
#       (dominant *visible* color from entity overrides: TrueColor > ACI > ByLayer)
# - Auto-split uploads:
#     * Main tab  → non-LAYER rows (counts etc.)
#     * ByLayer   → only LAYER_SUMMARY/qty_type='layer'
# - ByLayer previews are EMBEDDED (no Drive URL needed) for reliability.
#
...
import requests
import ezdxf
from ezdxf import colors as ezcolors  # ACI/TrueColor helpers
...
GS_DRIVE_FOLDER_ID  = ""          # Drive folder for previews ("" → auto-create)
# ===============================================================
...
def _render_preview_from_insert(ins, size_px:int=192, pad_ratio:float=0.06) -> str:
    """Geometry preview (used for DETAIL sheet only)."""
    try:
...
def _make_color_swatch_b64(rgb: tuple[int,int,int], size_px:int=192) -> str:
    """Solid color swatch PNG for ByLayer sheet — non-transparent, reliable."""
    try:
        r,g,b = rgb
        fig = plt.figure(figsize=(size_px/100, size_px/100), dpi=100)
        fig.patch.set_facecolor((r/255.0, g/255.0, b/255.0))
        ax = fig.add_axes([0,0,1,1])
        ax.set_xlim(0,1); ax.set_ylim(0,1)
        ax.axis("off")
        ax.set_facecolor((r/255.0, g/255.0, b/255.0))
        buf = io.BytesIO()
...
# Base map from LayerTable (fallback color)
...
# --- Color resolution & dominant color voting -------------------------------
def _resolve_entity_rgb(e, layer_rgb_map: Dict[str, tuple[int,int,int]]) -> tuple[int,int,int]:
    """Return the *visible* RGB of an entity: TrueColor > explicit ACI > ByLayer."""
...
def _entity_weight_for_colorvote(e) -> float:
    """Weight an entity's color contribution. Length for curves; area for hatches."""
    et = e.dxftype()
    try:
        if et == "LINE":
...
def _dominant_layer_rgb_map(msp, base_layer_rgb: Dict[str, tuple[int,int,int]], scale_to_m: float) -> Dict[str, tuple[int,int,int]]:
    ...
    def _acc(e):
        ...
        # Normalize units for relative voting (m or m²)
        if e.dxftype() == "HATCH":
            w *= (scale_to_m**2)
        else:
            w *= scale_to_m
    ...
    out = dict(base_layer_rgb)  # start with layer table colors
...
# --- Recover rectangle dimensions from perimeter & area ---
...
def make_layer_total_rows(open_by, peri_by, area_by, run_id, source_file,
                          layer_rgb: Dict[str, tuple[int,int,int]] | None = None,
                          mode: str = "split"):
    """
    Returns layer summary rows. For CLOSED rows we also compute rectangle dims.
    If layer_rgb is provided, attaches a color swatch preview_b64 for each layer row.
    """
    rows = []
    all_layers = sorted(set(open_by.keys()) | set(peri_by.keys()) | set(area_by.keys()))
    layer_rgb = layer_rgb or {}

    def swatch_for(ly: str) -> str:
        rgb = layer_rgb.get(ly)
        return _make_color_swatch_b64(rgb) if rgb else ""
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for r in rows:
            writer.writerow({
                ...
                "length":     r.get("bbox_length",""),
                "width":      r.get("bbox_width",""),
                "perimeter":  r.get("perimeter",""),
                "area":       r.get("area",""),
                "Preview":    "",  # Web App fills IMAGE()/embed from uploaded file
...
def push_rows_to_webapp(rows: list[dict], webapp_url: str, spreadsheet_id: str,
    ...
    for idx, i in enumerate(range(0, total, batch_rows), start=1):
        ...
        # 👇 If these rows are for the layer sheet (LAYER_SUMMARY), embed the images directly
        first_is_layer = bool(rows) and (rows[0].get("entity_type") == "LAYER_SUMMARY")
        ...
        payload = {
            ...
            "embedImages": True if first_is_layer else False,  # embed on ByLayer tab
            "imageMode": 4,   # used only if embedImages is False
            "imageW": 28,
            "imageH": 28,
        }
...
def process_one_dxf(dxf_path: Path, out_dir: Path | None,
    ...
    # Per-layer metrics + colored swatches (dominant visible color)
    if layer_metrics:
        open_by, peri_by, area_by = compute_layer_metrics(msp, scale_to_m, target_units)
        base_layer_rgb = _layer_rgb_map(doc)                # LayerTable colors
        dom_layer_rgb  = _dominant_layer_rgb_map(msp, base_layer_rgb, scale_to_m)  # overrides-resolved
...
def main():
    ...
    ap.add_argument("--gsheet-summary-tab", default=None, help="Optional summary tab (default: auto '<MainTab>_ByLayer' if blank).")
    ...
    ap.add_argument("--drive-folder-id", default=None, help="Google Drive folder ID to store preview PNGs.")
    ...
    if explicit_out:
        ...
        if gs_webapp and gsheet_id:
            ...
            target_summary_tab = _summary_tab_name()
            if layer_rows:
                push_rows_to_webapp(layer_rows, gs_webapp, gsheet_id, target_summary_tab,
