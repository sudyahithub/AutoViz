...
# - INSERT aggregation by (block_name, PLANNER, zone) using median bbox (length/width)
# - New 'category1' column (left of BOQ name) = original DWG layer
# - Removed run_id/source_file/handle from export
# - Per-layer metrics (unchanged)
# - Detail (INSERT) previews stay working
# - ZONING: detect PLANNER rectangles, name from PLANNER block or label, assign inserts by bbox center
...
# ---- Web App defaults (override via CLI if needed)
...
# ---- Output format (FIRST THREE REMOVED). New column order:
# entity_type | category | zone | category1 | BOQ name | qty_type | qty_value | length | width | perimeter | area | Preview | remarks
CSV_HEADERS = [
    ...
    for ins in msp.query("INSERT"):
        try:
            ...
            out.append(make_row(
                ...
                zone=center_zone,
                category1=(ly or "").strip().lower(),     # NEW: original layer → category1
                remarks=remarks_txt
            ))
...
def push_rows_to_webapp(rows: list[dict], webapp_url: str, spreadsheet_id: str,
    ...
    headers = CSV_HEADERS[:]
    data_rows = [[
        r.get("entity_type",""), r.get("layer",""), r.get("zone",""), r.get("category1",""),
        r.get("block_name",""), r.get("qty_type",""), r.get("qty_value",""),
        r.get("bbox_length",""), r.get("bbox_width",""), r.get("perimeter",""), r.get("area",""),
        "", r.get("remarks",""),
    ] for r in rows]
    ...
    total = len(data_rows); sent = 0
    for idx, i in enumerate(range(0, total, batch_rows), start=1):
        row_objs     = rows[i:i+batch_rows]
        chunk_rows   = data_rows[i:i+batch_rows]
        chunk_imgs   = [r.get("preview_b64","") for r in row_objs]   # Detail: base64 previews
        chunk_colors = [r.get("preview_hex","") for r in row_objs]   # ByLayer: hex colors

        is_layer = bool(row_objs) and (row_objs[0].get("entity_type") == "LAYER_SUMMARY")
        if is_layer:
            chunk_imgs = [""] * len(chunk_rows)  # no images for ByLayer
        ...
        payload = {
            ...
            "rows": chunk_rows,
            "images": chunk_imgs,
            "bgColors": chunk_colors,
            "colorOnly": True if is_layer else False,   # ByLayer → color cell
            "embedImages": False,                       # Detail → =IMAGE(DriveURL) handled in Web App
...
def main():
    ...
    ap.add_argument("--decimals", type=int, default=None, help="Decimal places for numeric outputs (e.g., 2)")
