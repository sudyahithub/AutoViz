#!/usr/bin/env python3
# dxf_to_boq_csv.py — DXF → CSV + Google Sheets (Apps Script Web App)
#
# - Aggregates INSERTs by (block_name, layer) with median bbox (length/width)
# - Per-layer totals: one row per layer; total length → 'length', total area → 'width'
# - Safe numeric formatting (no float("") crashes)
# - Uploads to Google Sheets via Apps Script Web App (requests + batching)
# - Sheet display polish controlled from CLI:
#     --align-middle        : vertical-align middle (Apps Script applies)
#     --sparse-anchor last  : place category label at first|last|middle of each block
#
# Install deps:  pip install ezdxf requests
...
def make_layer_total_rows(L_by, A_by, run_id, source_file, fill_zeros=False):
    ...
    for ly in all_layers:
        ...
        rows.append(make_row(
            ...
            bbox_length=(L if (fill_zeros or L>0) else None),
            bbox_width=(A if (fill_zeros or A>0) else None)
        ))
    return rows
...
def push_rows_to_webapp(rows: list[dict], webapp_url: str, spreadsheet_id: str,
                        ...
                        batch_rows: int = 3000, timeout: int = 120,
                        valign_middle: bool = False, sparse_anchor: str = "last") -> None:
    ...
    for idx, chunk in enumerate(_chunk(data_rows, batch_rows), 1):
        payload = {
            ...
            "headers": headers if (idx == 1 and first_mode == "replace") else [],
            "rows": chunk,
            # display preferences the Web App will apply
...
# ---------------------------
def main():
    ap = argparse.ArgumentParser(description="DXF → CSV (INSERT bbox + per-layer totals) + Google Sheets Web App upload.")
