#!/usr/bin/env python3
# dxf_to_boq_csv.py  — robust uploader + batching
#
# Features
# - INSERT bbox (length/width), aggregated by (block_name, layer) with median size
# - Per-layer totals: ONE row per layer (length in 'length' col, area in 'width' col)
# - Blank layer → category = "misc"
# - Writes CSV locally
# - Pushes to Google Sheets via Apps Script Web App using 'requests' (handles 302)
# - Batches large uploads automatically
#
# Requires: pip install ezdxf requests
...
import requests     # <— new; replaces urllib for reliability
import ezdxf
...
# ---- safe number formatting (prevents float("") crashes)
...
def make_layer_total_rows(L_by, A_by, run_id, source_file, fill_zeros=False):
    rows = []
    all_layers = sorted(set(L_by.keys()) | set(A_by.keys()))
    for ly in all_layers:
        L = L_by.get(ly, 0.0)
        A = A_by.get(ly, 0.0)
        length_cell = L if (fill_zeros or L > 0) else None
        area_cell   = A if (fill_zeros or A > 0) else None
        rows.append(make_row(
            run_id, source_file,
            entity_type="LAYER_SUMMARY",
            qty_type="layer",
            qty_value=None,              # leave numeric cell empty in qty_value
            block_name="",
            layer=ly,
            remarks="totals per layer (length & area)",
            bbox_length=length_cell,     # -> 'length' column
            bbox_width=area_cell         # -> 'width' column (area)
        ))
    return rows
...
def push_rows_to_webapp(rows: list[dict], webapp_url: str, spreadsheet_id: str,
                        ...
                        summary_tab: str = "", batch_rows: int = 3000,
                        timeout: int = 120) -> None:
    """POST rows to Apps Script Web App using requests; batches large payloads."""
    ...
    total = len(data_rows)
    sent = 0
    for idx, chunk in enumerate(_chunk(data_rows, batch_rows), 1):
        payload = {
            "sheetId": spreadsheet_id,
            "tab": tab,
            "mode": "replace" if (idx == 1 and first_mode == "replace") else "append",
            "headers": headers if (idx == 1 and first_mode == "replace") else [],  # set headers only on first replace
            "rows": chunk,
        }
        if summary_tab and idx == 1:
            payload["summaryTab"] = summary_tab
            payload["summaryRows"] = []  # not used in this build

        r = sess.post(webapp_url, json=payload, timeout=timeout, allow_redirects=True)
        if not r.ok:
...
def process_one_dxf(dxf_path: Path, out_dir: Path | None,
    ...
    # INSERT rows (then aggregate by name+layer with median size)
    ...
    # Per-layer metrics → ONE row per layer with both numbers
    if layer_metrics:
        L_by, A_by = compute_layer_metrics(msp, scale_to_m, target_units)
        rows.extend(make_layer_total_rows(L_by, A_by, run_id, source_file, fill_zeros=False))
...
# ---------------------------
def main():
    ap = argparse.ArgumentParser(description="DXF → CSV (INSERT bbox + per-layer totals) + Google Web App upload.")
    ...
    if explicit_out:
        ...
        if gs_webapp and gsheet_id:
            push_rows_to_webapp(rows, gs_webapp, gsheet_id, gsheet_tab, gsheet_mode, gsheet_summary_tab, batch_rows=batch_rows)
        return
    ...
    if all_rows and gs_webapp and gsheet_id:
        push_rows_to_webapp(all_rows, gs_webapp, gsheet_id, gsheet_tab, gsheet_mode, gsheet_summary_tab, batch_rows=batch_rows)
