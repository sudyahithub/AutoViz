...
# White linework on transparent background. Robust trimming, masking, and
# safe mask shaping to avoid (H,W,1) -> (H,W) broadcast errors.
...
TARGET_SIZE    = 512      # default final width & height (can be overridden by --size)
# -------------------------------------
...
# ===== Visibility controls =====
INK_THRESHOLD     = 252   # captures faint anti-aliased pixels
THICKEN_PX        = 3     # pre-resize dilation kernel
THICKEN_ITER      = 2
CLOSE_GAPS_KSIZE  = 2

# Post-resize stroke boost (scales with final size; base tuned for 128)
POST_THICKEN_PX_BASE   = 1
POST_THICKEN_ITER      = 1
...
def _apply_glow(out_rgba, alpha_mask, blur_px=2, glow_alpha=90):
    """Add a soft white halo under the strokes (safe 2D alpha write)."""
    out_rgba = _ensure_rgba(out_rgba)
    a_in = _as_single_channel(alpha_mask)
    if blur_px <= 0 or glow_alpha <= 0:
        return out_rgba

    glow = cv2.GaussianBlur(a_in, (0, 0), blur_px).astype(np.uint8)
    ...
    out[..., :3] = np.clip(rgb, 0, 255).astype(np.uint8)     # (H,W,3)
    alpha_out = np.clip(a * 255, 0, 255).astype(np.uint8)    # (H,W,1)
    alpha_out = _as_single_channel(alpha_out)                # -> (H,W)
    out[..., 3] = alpha_out
    return out
...
def trim_and_to_rgba_white_lines(path_in: str, target_size: int):
    """
    Pipeline:
      - Transparent render -> trim -> margin
      - Edge-connected background removal
      - Ink mask build + gap close + pre-thicken
      - Draw white RGBA -> resize to N -> post-thicken -> optional glow
    """
    ...
    # 3) Background = edge-connected near-white OR low-alpha
    ...
    # 4) Ink = not background AND (alpha >= min OR gray darker than threshold)
    ...
    # 7) Post-resize thicken (scaled for size)
